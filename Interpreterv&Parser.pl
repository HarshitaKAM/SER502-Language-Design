...
evalI(letter(X), StartEnv, Val) :- lookUp(X, StartEnv, Val).									

evalE(arithexp(X), StartEnv, Val) :- evalEx(X, StartEnv, Val).

evalEx(express(X), StartEnv, Val) :- evalFactor(X, StartEnv, Val).

evalFactor(factor(X), StartEnv, Val) :- evalNum(X, StartEnv, Val).

evalNum(number(X), StartEnv, Val) :- lookUp(X, StartEnv, Val).
